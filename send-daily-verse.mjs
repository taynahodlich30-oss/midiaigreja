...
if (!process.env.FIREBASE_SERVICE_ACCOUNT) throw new Error('O segredo FIREBASE_SERVICE_ACCOUNT não foi configurado.');
admin.initializeApp({ credential: admin.credential.cert(JSON.parse(process.env.FIREBASE_SERVICE_ACCOUNT)) });
const db = admin.firestore();
const messaging = admin.messaging();
// Textos curtos em formulação tradicional de domínio público; rotação diária determinística.
const verses = [
...
for (const userDoc of users.docs) {
  const devices = await userDoc.ref.collection('devices').where('enabled','==',true).get();
  const tokens=devices.docs.map(d=>d.data().token).filter(Boolean);
  if (!tokens.length) continue;
  const result=await messaging.sendEachForMulticast({tokens,notification:{title,body},data:{type:'verse',url:'https://taynahodlich30-oss.github.io/midiaigreja/'},webpush:{fcmOptions:{link:'https://taynahodlich30-oss.github.io/midiaigreja/'}}});
  if (result.successCount) {
    await userDoc.ref.collection('notifications').add({type:'verse',title,body,reference:ref,createdAt:admin.firestore.FieldValue.serverTimestamp()});
